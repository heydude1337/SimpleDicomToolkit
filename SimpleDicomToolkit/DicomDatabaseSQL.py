...
class Database(sdtk.Logger, HeadersForDatabase):
    ...
    def get_column(self, column_name, distinct=True,
        ...
        values = self.database.get_column(self.builder._MAIN_TABLE, column_name,
                                    sort_by=sort_by, distinct=distinct, close=False,
                                    **{self.builder._SELECT_COL: True})
    ...
    def select(self, close=True, partial_match=False, **kwargs):
    ...
    def _encode_value(tagname, value):
        ...
        return sdtk.Encoder._convert_value(value, VR=VR, VM=VM)
...
class DatabaseBuilder(sdtk.Logger):
    ...
    def __init__(self, path=None, scan=True, silent=False, database_file=None,
        ...
        if rebuild and not in_memory:
            self.delete_database()
            
        self._init_database(database_file=database_file)
    ...
    def delete_database(self):
        self.logger.info('Removing: ' + self.database_file)
...
#            raise RuntimeError('No path specified')
    def _init_database(self, database_file = None):
        # make connection to the database and make columns if they don't exist
        self.database = sdtk.SQLiteWrapper(database_file)
    ...
    def _update_db(self, files= [], existing_files=None, silent=False):
        ...
        if existing_files is None:
            existing_files = self.database.get_column(self._MAIN_TABLE, 
                                                      self._FILENAME_COL)
