...
class Header(dict):
    ...
    def factory(iterable):
        # convert an iterables of dicts, skip already converted dicts
...
class DicomFiles(OrderedDict, DicomReadable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for value in self.values():
            [tag_names.add(k) for k in value.keys()]
    ...
    def __setattr__(self, attr, value):
        super(DicomFiles, self).__setattr__(attr, value)
...
def dicom_dataset_to_dict(dicom_header, skip_private_tags = True):
    ...
    for dicom_value in dicom_header.values():    
        ...
        if type(dicom_value) == dicom.dataelem.DataElement:
        ...
        elif type(dicom_value) == dicom.dataset.Dataset:  
            print('2')
        ...
        dicom_dict = Header.factory(dicom_dict)
...
#
#def decode(json_str):
#    return dict_with_attributes(json.loads(json_str))